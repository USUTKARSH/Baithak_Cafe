...
$('#item-form').addEventListener('submit', async event => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); const item = { id: data.id ? Number(data.id) : Date.now(), name: data.name, description: data.description, price: Number(data.price), category: data.category, image: data.image || 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=500&q=80' }; const existing = menu.findIndex(menuItem => menuItem.id === item.id); existing >= 0 ? menu.splice(existing, 1, item) : menu.push(item); saveMenu(); renderMenu(); await syncAdmin(existing >= 0 ? `/api/admin/menu/${item.id}` : '/api/admin/menu', { method: existing >= 0 ? 'PATCH' : 'POST', body: JSON.stringify(item) }); $('#item-modal').hidden = true; });
